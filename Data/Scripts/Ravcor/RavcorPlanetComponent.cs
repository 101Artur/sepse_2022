...
namespace Ravcor
{
    ...
    public sealed class RavcorPlanetComponent : MyGameLogicComponent
    {
        public class Config
        {
            [XmlElement]
            public float LargeShipDamagePs = 20000f;

            [XmlElement]
            public float SmallShipDamagePs = 1000f;

            [XmlElement]
            public float PlayerDamagePs = 50f;

            [XmlElement]
            public double DamageDistance = 5d;
        }
        ...
        const string PLANET_NAME = "Ravcor";
        const string LAVA_MATERIAL_NAME = "RavLava";
        ...
        public override void Init(MyObjectBuilder_EntityBase objectBuilder)
        {
            ...
            if (!TryGetMaterialIndexByName(LAVA_MATERIAL_NAME, out _lavaMaterialIndex))
            {
                Log($"Init() failed; Lava material not found: {_lavaMaterialIndex}");
                return;
            }
        ...
        void EnqueueCharacterDamage(IMyCharacter character)
        {
            ...
            if (material.Index != _lavaMaterialIndex) return;

            _damageQueue.Enqueue(new DamageState(character, _config.PlayerDamagePs));
        }
        ...
        void EnqueueGridDamage(IMyCubeGrid grid)
        {
            ...
            var damage = grid.GridSizeEnum == MyCubeSize.Small
                ? _config.SmallShipDamagePs
                : _config.LargeShipDamagePs;
            ...
            foreach (var block in _tmpBlocks)
            {
                ...
                if (material.Index != _lavaMaterialIndex) continue;

                var blockSurfaceDistance = Vector3D.Distance(blockPos, surfacePos);
                if (blockSurfaceDistance > _config.DamageDistance) continue;

                _damageQueue.Enqueue(new DamageState(block, damage));
            }
        }
        ...
        {
            MyLog.Default.Info($"{nameof(RavcorPlanetComponent)}: {message}");
        }
    }
}